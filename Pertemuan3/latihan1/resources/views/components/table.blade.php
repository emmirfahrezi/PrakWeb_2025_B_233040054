            ...
            <ul class="flex space-x-2 text-sm">
                ...
                @foreach ($posts->getUrlRange(1, $posts->lastPage()) as $page => $url)
                    @if ($page == $posts->currentPage())
                        <li>
                            <a href="{{ $url }}" aria-current="page" class="flex items-center justify-center text-fg-brand bg-neutral-tertiary-medium box-border border border-brand hover:border-brand hover:text-fg-brand font-medium text-sm w-10 h-10 focus:outline-none">{{ $page }}</a>
                        </li>
                    @else
                        <li>
        ...
        <tbody>
          @forelse($posts as $post)
            <tr class="bg-neutral-primary border-b border-default">
                ...
                <td class="px-6 py-4">
                    @if($post->image)
                      <img src="{{ asset('storage/' . $post->image) }}" alt="{{ $post->title }}" class="w-16 h-10 object-cover rounded-base">
                    @else
                      <span class="text-body">No Image</span>
                    @endif
                </td>
                <td class="px-6 py-4">
                    {{ $post->title }}
